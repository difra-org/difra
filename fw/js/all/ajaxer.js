...
ajaxer.process = function (data, form) {
	...
	try {
		...
		for (var key in dataObj.actions) {
			...
			if (typeof debug !==
				'undefined') {
				debug.addReq('Processing ajaxer method: ' +
					action.action);
			}
			ajaxer.triggerHandler('ajaxer-pre-' +
				action.action);
			switch (action.action) {
				...
				case 'exec':		// never use this if unsure, it's here only for very special cases
					...
					break;
				default:
					console.warn('Ajaxer action "' +
						action.action +
						'" not implemented');
			}
			ajaxer.triggerHandler('ajaxer-' +
				action.action);
		}
	} catch (err) {
		...
		if (debug !==
			undefined) {
			debug.addReq('Server returned data ajaxer could not parse: ' +
				data);
		}
	}
	if (typeof debug !=
...
ajaxer.notify = function (lang, message) {
	...
	ajaxer.overlayShow('<p>' +
		message +
		'</p>' +
		'<a href="#" onclick="ajaxer.close(this)" class="button">' +
		( lang.close ? lang.close : 'OK' ) +
		'</a>');
};
...
ajaxer.invalid = function (form, name, message) {
	...
	if (!el.length ||
		el.attr('type') ==
		'hidden') {
		ajaxer.error({}, 'Invalid value for field "' +
			name +
			'".');
		return;
	}
...
ajaxer.statusUpdate = function (form) {
	...
	$(form).find('.status').each(function (i, obj1) {
		...
		if (typeof obj.attr('original-text') ==
			"undefined") {
		...
		if (name in
			...
			obj.animate({opacity: 0}, 'fast', function () {
				if (obj.attr('status-class')) {
					if (obj.attr('status-class') !=
						ajaxer.statuses[name].classname) {
	...
	for (var i in ajaxer.statuses) {
		...
		if (!ajaxer.statuses[i].used) {
			//noinspection JSUnfilteredForInLoop
			console.warn('Status for ' +
				ajaxer.statuses[i].classname +
				': ' +
				ajaxer.statuses[i].message);
...
ajaxer.overlayShow = function (content) {
	...
	$('#ajaxer-' +
		ajaxer.id).find('.overlay-inner').fadeIn('fast');
	$(window).resize();
	ajaxer.id++;
};
...
ajaxer.initIframe = function (form, event) {
	...
	frame.one('load', function () {
		...
		var upprog = $('#upprog');
		if (upprog.length) {
			loading.find('td1').css('width', Math.ceil($('#upprog').width() -
					20) +
				'px');
		}
		loading.hide();
...
ajaxer.fetchProgress = function (uuid) {
	...
	if (res.state ==
		...
		progressbar.find('.td1')
			.css('width', Math.ceil(( progressbar.width() -
				20 ) *
				res.received /
				res.size) +
			'px');
...
ajaxer.watcher = function () {
	...
	if (mc) {
		...
		$.cookie('query', null, {
			path: "/",
			domain: config.mainhost ? '.' +
			config.mainhost : false
		});
	}
	mc = $.cookie('notify');
	if (mc) {
