...
class Blog {
	...
	private function _save() {
		...
		$keys = array( 'name' ); // свойства, которые нужно сохранять
		$sets = array();
		$db   = Difra\MySQL::getInstance();
		foreach( $keys as $key ) {
			if( !is_null( $this->$key ) ) {
	 ...
	 *
	 * @return array        массив: 'posts' => массив постов, 'total' => количество записей, 'pages' => количество страниц
	 */
	public function getPosts( $page = 1, $perPage = null, $hidden = false, $noNickName = false ) {
		...
		$query1 =
			' FROM `blogs_posts` LEFT JOIN `users_fields` ON `blogs_posts`.`user`=`users_fields`.`id` WHERE ' . implode( ' AND ', $cond )
			. $tagsLimit;
		$query  =
		...
		// считаем комментарии
		if( !empty( $data ) ) {
			$comments = $db->fetch( 'SELECT `parent_id` AS `id`,count(`parent_id`) AS `count` FROM `blogs_comments` WHERE `parent_id` IN ('
	...
	public function getPost( $id ) {
		...
		$query = 'SELECT `blogs_posts`.`id`,`blog`,`user`,`title`,`link`,`preview`,`date`,`visible`,`text`,' . '`users_fields`.`value` as `nickname` '
	...
	public function addPost( $userId, $title, $text, $hidden = false ) {
		...
		return Post::add( $this->id, $userId, array(
