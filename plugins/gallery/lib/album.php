...
class Album {
	...
	public function load() {
		...
		$data = $db->fetchRow( "SELECT `name`,`description`,`visible` FROM `gallery_albums` WHERE `id`='" . $db->escape( $this->id ) . "'" );
		if( empty( $data ) ) {
			return false;
		}
		$this->name = $data['name'];
	...
	public function getImages() {
		...
		$this->load();
		$db = \Difra\MySQL::getInstance();
		return $db->fetch( "SELECT `id` FROM `gallery_photos` WHERE `album`='" . $db->escape( $this->id ) . "' ORDER BY `position`" );
	}
