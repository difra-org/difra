...
class Radio {
	...
	private $validSampleRate = array( 22100, 44100, 48000, 96000, 192000 );
	...
	public function addToChannel( $tracks, $channel ) {
		...
		if( !empty( $insertArray ) ) {
			$query = "INSERT IGNORE INTO `radio_tracks` (`id`, `channel`, `duration`, `group_id`) VALUES " . implode( ', ', $insertArray );
			$db->query( $query );
			return true;
		}
		return false;
	...
	public function makeIceCast() {
		...
		if( !empty( $res ) ) {
			...
			foreach( $res as $data ) {
				...
				$mountNode->appendChild( $originalXML->createElement( 'fallback-mount', '/' . $data['mount'] . '-nonstop' ) );
				...
				$fallBackNode->appendChild( $originalXML->createElement( 'mount-name', '/' . $data['mount'] . '-nonstop' ) );
