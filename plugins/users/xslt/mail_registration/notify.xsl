...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  version="1.0">	
	...
	<xsl:template match="/mail">
		...
		<subject><xsl:value-of select="/mail/locale/auth/mail/registration/subject"/></subject>	
		<text>
			...
			<xsl:choose>
				...
				<xsl:when test="@confirm='email'">
					...
					<a href="http://{@host}/auth/activate/{@code}">http://<xsl:value-of select="@host"/>/auth/activate/<xsl:value-of select="@code"/></a>
