...
class Portfolio {
	...
	public function saveContributor( $data ) {
		...
		Portfolio\Contributors::saveContributor( $data['user'], $data['name'], $data['linktext'],
			null, $data['role'], $archive );
	}
    ...
    public function savePreviewImage( $workId, $previewImage ) {
        ...
        $smallImg = \Difra\Libs\Images::getInstance()->createThumbnail( $previewImage,
                $this->settings['thumb_maxWidth'], $this->settings['thumb_maxHeight'], 'jpg' );
