...
class BlogsPostController extends Difra\Controller {
		...
		if( $group = Blogs\Group::current() ) {
			if( !$group->hasUser( $userId ) ) {
				\Difra\Libs\Cookies::getInstance()->notify( \Difra\Locales::getInstance()
										    ->getXPath( 'blogs/notifies/add_post_denied' ), true );
				$this->view->redirect( '/' );
			}
	...
	public function editActionAuth( Param\AnyInt $id ) {
		...
		if( $post->getUser() != $Auth->getId() && !$Auth->isModerator() ) {
			...
			if( !$group or $group->getOwner() != \Difra\Auth::getInstance()->getId() ) {
				\Difra\Libs\Cookies::getInstance()->notify( \Difra\Locales::getInstance()->getXPath( 'blogs/notifies/edit_post_denied' ),
									    true );
				$this->view->redirect( '/' );
			}
		}
	...
	public function updateAjaxActionAuth( Param\AjaxInt $id, Param\AjaxString $title, Param\AjaxSafeHTML $text, Param\AjaxString $tags = null ) {
		...
		if( $post->getUser() != $Auth->getId() && !$Auth->isModerator() ) {
			...
			if( !$group or $group->getOwner() != \Difra\Auth::getInstance()->getId() ) {
				\Difra\Libs\Cookies::getInstance()->notify( \Difra\Locales::getInstance()
										    ->getXPath( 'blogs/notifies/edit_post_denied' ), true );
				$this->view->redirect( '/' );
			}
		}
	...
	public function deletenotifyAjaxActionAuth( Param\AjaxString $id = null ) {
		...
		$this->ajax->display( Difra\Locales::getInstance()->getXPath( 'blogs/notifies/delete_post' ) . '<br/><br/><div href="#" onclick="blogs.delete( '
