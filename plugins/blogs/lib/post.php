...
class Post {
	 ...
	 * @return Post
	 */
	public static function add( $blogId, $userId, $data = array() ) {

		$post       = new self;
		$post->blog = intval( $blogId );
		$post->user = intval( $userId );
		$keys       = array( 'title', 'text', 'visible' );
		foreach( $keys as $key ) {
			if( $data[$key] != '' ) {
				$post->$key = $data[$key];
			}
	...
	public function makePreview() {
		...
		$this->preview = \Difra\Param\Filters\HTML::getInstance()->process( $preview, false, false );
	}
	...
	public function getUrl() {
		...
		if( !$this->groupDomain ) {
			return 'http://' . \Difra\Site::getInstance()->getMainhost() . '/blogs/' . rawurlencode( $this->nickname ) . '/'
			       . $this->id . '/' . $this->getLink();
		} elseif( $this->blog != 1 ) {
			return 'http://' . $this->groupDomain . '.' . \Difra\Site::getInstance()->getMainhost() . '/' . $this->id . '/'
			       . $this->getLink();
		} else {
			return 'http://' . \Difra\Site::getInstance()->getMainhost() . '/' . $this->id . '/' . $this->getLink();
		}
	}
