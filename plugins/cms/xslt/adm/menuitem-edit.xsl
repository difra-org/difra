...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" version="1.0">
	...
	<xsl:template name="CMSMenuItem">
		...
		<div id="pageForm" class="menuItemForm">
			<xsl:choose>
				<xsl:when test="page">
					<form action="/adm/cms/items/savepage" method="post" class="ajaxer">
						<xsl:choose>
		...
		<div id="linkForm" style="display:none" class="menuItemForm">
			<form action="/adm/cms/items/savelink" method="post" class="ajaxer">
				<xsl:choose>
					<xsl:when test="name()='CMSMenuItemAdd'">
						<input type="hidden" name="menu" value="{@id}"/>
					</xsl:when>
				...
				<table class="form">
					<tr>
						...
						<td>
							<input type="text" name="label" value="{@linkLabel}"/>
						</td>
					</tr>
					<tr>
						<th>
