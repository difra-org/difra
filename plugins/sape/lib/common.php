...
abstract class Common {
	...
	protected static function isSapeBot() {
		...
		static $isBot = null;
		return !is_null( $isBot ) ? $isBot : $isBot = ( !empty( $_COOKIE['sape_cookie'] ) and $_COOKIE['sape_cookie'] == self::getSapeUser() );
	}
	...
	protected static function isForcedUpdate() {
		...
		return !is_null( $forcedUpdate ) ? $forcedUpdate :
			$forcedUpdate = ( self::isSapeBot() and !empty( $_COOKIE['sape_updatedb'] ) and $_COOKIE['sape_updatedb'] == 1 );
	}
