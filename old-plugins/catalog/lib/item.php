...
class Item
{
    ...
    public function load()
    {
        ...
        $this->humanDate = Locales::getInstance()->getDateFromMysql($data['created']);
        $this->loaded = true;

        $db = MySQL::getInstance();
        $imgData =
            $db->fetch('SELECT `id`,`main` FROM `catalog_images` WHERE `item`=\'' . $db->escape($this->id) . "'");
        if (!empty($imgData)) {
