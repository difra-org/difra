...
class Comment
{
	...
	private function sendMail()
	{
		...
		if (!isset($userAdditionals['unsubscribe']) || $userAdditionals['unsubscribe'] == 0) {
			...
			$query = "SELECT `email`, `activation` FROM `users` WHERE `id`='" . intval($elementOwner) .
			...
			if ($this->replyId) {
				...
				$query = "SELECT c.`text`, c.`user` FROM `" . $this->module . "_comments` c WHERE c.`id`='" .
						 $this->replyId . "'";
			...
			if (!empty($userData)) {
				...
				if ($this->module == 'albums') {
					...
					if (isset($res['domain']) && $res['domain'] != '') {
						...
						$elementLink =
							'http://' . \Difra\Site::getInstance()->getMainhost() . '/blogs/' . $ownerNickname .
							'/' . $this->moduleId . '/' . rawurlencode($res['link']) . '/';
					}
				}

				$unsubscribeLink =
					'http://' . \Difra\Site::getInstance()->getMainhost() . '/unsubscribe/' . $userData['activation'] .
					'/';
	...
	public static function getCommentsCount($module, $moduleId)
	{
		...
		$query =
			"SELECT COUNT( `id` ) AS `count` FROM `{$module}_comments` WHERE `parent_id`='" . intval($moduleId) . "'";
		$res = $db->fetchRow($query);
	...
	public static function getCommentsXML($node, $module, $moduleId)
	{
		...
		$commentsData =
			$db->fetch("SELECT `{$module}_comments`.*,`users_fields`.`value` AS `nickname` FROM `{$module}_comments` "
