...
class Channel
{
	...
	private function _getLastPosition()
	{
		...
		$db = \Difra\MySQL::getInstance();
		$r = $db->fetchRow("SELECT MAX(`position`) AS `position` FROM `radio_playlist` WHERE `channel` = '" .
						   $this->name . "'");
		if ($r['position'] > 0) {
			return $r['position'];
		}
	...
	private function _generatePlayList()
	{
		...
		$query[] =
			"UPDATE `radio_tracks` SET `lastPlayed` = NOW() WHERE `id` = '" . $track['id'] . "' AND `channel`='" .
			$this->name . "'";
		$query[] = "UPDATE `radio_tracks` SET `lastPlayedArtist` = NOW()
	...
	public function setSettings($mount, $data)
	{
		...
		$query =
			"UPDATE `radio_channels` SET " . implode(', ', $insertData) . " WHERE `mount`='" . $db->escape($mount) .
			"'";
		$db->query($query);
	...
	public function getTrackToPlay()
	{
		...
		$track = $db->fetchRow("SELECT * FROM `radio_playlist` WHERE `channel`='" . $this->name .
		...
		$nowPlayArray = [
			...
			'start' => time(),
			'stop' => time() + $track['duration']
		];

		$query[] = "REPLACE INTO `radio_temp_data` (`channel`, `name`, `data`)
				VALUES ('" . $this->name . "', 'currentPlay', '" . $db->escape(serialize($nowPlayArray)) . "')";
		$query[] = "DELETE FROM `radio_playlist` WHERE `channel`='" . $this->name . "' AND `position`='" .
				   intval($track['position']) . "'";
		$query[] = "UPDATE `radio_playlist` SET `position` = `position`-1 WHERE `channel` = '" . $this->name .
	...
	public function setPlayList($order)
	{
		...
		if (!empty($tracks)) {
			...
			foreach ($order as $trackId) {
				...
				$valuesArray[] =
					"('" . $p . "', '" . $this->name . "', '" . $trackId . "', '" . $tracks[$trackId]['group_id'] . "',
