...
class Tags
{
    ...
    public function saveTags($module, $tags)
    {
        ...
        foreach ($tags as $tag) {
            $query[] =
                "INSERT IGNORE INTO `{$module}_tags` SET `tag`=" . $tag . ", `link`='" . $this->_makeLink($tag) . "'";
        }
        $db->query($query);
    ...
    public function assignTags($module, $tagsId, $itemId)
    {
        ...
        foreach ($tagsId as $tagId) {
            $query[] = "INSERT IGNORE INTO `{$module}_to_tags` SET `tag_id`='" . intval($tagId) . "', `item_id`='" .
                       intval($itemId) . "'";
        }
        $db->query($query);
    }
    ...
    public function getCloudXml($module, $node, $limit = 25)
    {
        ...
        if (!$res = $cache->get($cacheKey)) {
            $db = Difra\MySQL::getInstance();
            $res =
                $db->fetch("SELECT `tag`, `link`, `weight` FROM `{$module}_tags` WHERE `cloud`=1 ORDER BY `tag` ASC LIMIT " .
                           $limit);
