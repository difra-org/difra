...
Class Announcement
{
    ...
    private function setObject($data)
    {
        ...
        if (isset($data['domain'])) {
            $this->groupData = ['name' => $data['name'], 'domain' => $data['domain'], 'id' => $data['group']];
        }

        if (isset($data['email'])) {
            $this->userData = ['id' => $data['user'], 'email' => $data['email'], 'nickname' => $data['nickname']];
        }
        ...
        if (isset($data['additionalData']) && !empty($data['additionalData'])) {
            ...
            foreach ($data['additionalData'] as $k => $addData) {
                $additionalArray[$addData['additional_id']] =
                    ['value' => $addData['value'], 'alias' => $addData['alias']];
            }
    ...
    public static function getById($id)
    {
        ...
        $query =
            "SELECT an.*, u.`email`, uf.`value` AS `nickname`, anns.`schedule`, aloc.`locationData` " . $groupSelect . "
    ...
    public function setTitle($string)
    {
        ...
        $this->link = Locales::getInstance()->makeLink(trim($string));
    }
    ...
    {
        $this->fromEventDate = Locales::getInstance()->getMysqlDate($date);
    }
    ...
    {
        $this->eventDate = Locales::getInstance()->getMysqlDate($date);
    ...
    {
        $this->beginDate = Locales::getInstance()->getMysqlDate($date);
    }
    ...
    {
        $this->endDate = Locales::getInstance()->getMysqlDate($date);
    }
    ...
    public function save()
    {
        ...
        if (is_null($this->id)) {
            ...
            $query = "INSERT INTO `announcements` SET `user`='" .
                     $this->user .
                     "', `group`='" .
                     $this->group .
                     "', `title`='" .
                     ...
                     $db->escape($this->link) .
                     "', `shortDescription`='" .
                     $db->escape($this->shortDescription) .
                     "', `eventDate`='" .
                     $this->eventDate .
                     "', `beginDate`='" .
                     $this->beginDate .
                     ...
                     $this->fromEventDate .
                     "', `location`='" .
                     intval($this->location) .
                     "'";
        } else {
            $query = "UPDATE `announcements` SET `user`='" .
                     $this->user .
                     "', `group`='" .
                     $this->group .
                     "', `title`='" .
    ...
    {
        if ($this->description instanceof Param\AjaxHTML or
            $this->description instanceof Param\AjaxSafeHTML
        ) {
            $this->description->saveImages(
                DIR_DATA . 'announcements/img/' . $this->id,
                '/announcements-img/' . $this->id
            );
    ...
    public static function getAll($onlyVisible = false, $archive = false, $limit = 40)
    {
        ...
        if ($onlyVisible) {
            $where =
                " WHERE an.`visible`=1 AND an.`beginDate` <= NOW() AND an.`endDate` >= DATE_FORMAT(NOW(),'%Y-%m-%d 00:00:00')";
        }
        if ($archive) {
        ...
        $res = $db->fetch(
            <<<SQL
            SELECT
    `an`.*, `u`.`email`, `uf`.`value` AS `nickname`, `aloc`.`locationData` `$groupSelect`,
...
$groupJoin
$where
ORDER BY `eventDate` >= `curStamp` DESC, IF(`eventDate` >= `curStamp`, `sortDate`, ''), `sortDate` DESC, `priority` DESC
LIMIT $limit;
SQL
        );
        $eventsArray = false;
    ...
    public static function getByPriority($priority = 100)
    {
        ...
        $query = "SELECT an.*, u.`email`, uf.`value` AS `nickname` " .
                 $groupSelect .
                 "
                    FROM `announcements` an
                    LEFT JOIN `users` AS `u` ON u.`id`=an.`user`
                 ...
                 "
                    WHERE an.`priority`>='" .
                 intval($priority) .
                 "' AND an.`eventDate`>=NOW() AND an.`visible`=1 ORDER BY an.`eventDate` ASC";
        $res = $db->fetch($query);
    ...
    public static function getByGroup($groupId, $withArchive = false)
    {
        ...
        $query = "SELECT an.*, u.`email`, uf.`value` AS `nickname`, g.`name`, g.`domain`
                 ...
                 "' " .
                 $whereAddon .
                 " AND an.`visible`=1 ORDER BY an.`eventDate` ASC";
        $res = $db->fetch($query);
    ...
    public function getXML($node)
    {
        ...
        if (!is_null($this->fromEventDate) && $this->fromEventDate != ''
            ...
            $eventNode->appendChild(
                $node->ownerDocument->createElement('fromToEventDiff', $this->getEventPeriodDays())
            );
        }

        $dateNode = $eventNode->appendChild(
        ...
        $eventNode->appendChild(
            $node->ownerDocument->createElement('modified', $Locale->getDateFromMysql($this->modified, true))
        );
    ...
    public function getHumanizedTitle()
    {
        ...
        if ($this->fromEventDate != '0000-00-00' &&
            $this->fromEventDate != '' &&
            $this->fromEventDate != $this->eventDate
        ) {
            ...
            $title .= Locales::get(
                    "announcements/dates/months/*[name()='month_" .
                    $exFromEventDate[1] . "']"
                ) . ' ';
            ...
            $title .= Locales::get(
                    "announcements/dates/weekdays/*[name()='day_" .
                    $exEventDate[0] . "']"
                ) . ', ';
    ...
    {
        $title = '';
        if ($this->fromEventDate != '' &&
            $this->fromEventDate != '0000-00-00' &&
            $this->fromEventDate != $this->eventDate
        ) {
    ...
    private function _getIsoDate(\DOMNode $node)
    {
        ...
        if (!is_null($this->fromEventDate) &&
            !is_null($this->eventDate) &&
            $this->fromEventDate != '0000-00-00'
        ) {
