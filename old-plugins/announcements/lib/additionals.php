...
Class Additionals
{
    ...
    {
        $res = Cache::getInstance()->get('announcements_additionals');
        if ($res) {
            ...
            $res = $db->fetchOne(
                "SELECT `id` FROM `announcements_additionals` WHERE `alias`='" . $db->escape($alias) . "'"
            );
    ...
    {
        MySQL::getInstance()->query("DELETE FROM `announcements_additionals` WHERE `id`='" . intval($id) . "'");
        self::cleanCache();
    }
    ...
    public static function saveData($announceId, $dataArray)
    {
        if (!empty($dataArray)) {
            ...
            foreach ($dataArray as $addId => $value) {
                if ($value != '') {
                    $addArray[] =
                        "( '" . intval($announceId) . "', '" . intval($addId) . "', '" . $db->escape($value) . "' )";
                }
            }
            if ($addArray) {
                $query[] =
                    "DELETE FROM `announcements_additionals_data` WHERE `announce_id`='" . intval($announceId) . "'";
                $query[] =
                    "INSERT INTO `announcements_additionals_data` (`announce_id`, `additional_id`, `value`) VALUES "
                    . implode(",", $addArray);
                $db->query($query);
            }
        }
