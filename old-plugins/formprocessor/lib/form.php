...
class form
{
    ...
    public function setFormFields($fieldsArray)
    {
        ...
        foreach ($fieldsArray['types'] as $n => $type) {
            ...
            $fieldArray[$index] = [
                'type' => $type,
                'name' => $fieldsArray['names'][$n],
                'description' => $fieldsArray['descriptions'][$n]
            ];
    ...
    private function save()
    {
        ...
        if (is_null($this->id)) {
            ...
            $query = "INSERT INTO `fp_forms` (`uri`, `title`, `answer`, `submit`, `fields`)
                        VALUES ('" . $db->escape($this->uri) . "', '" . $db->escape($this->title) . "', '" .
                     $db->escape($this->answer) .
            ...
            $query =
                "UPDATE `fp_forms` SET `uri`='" . $db->escape($this->uri) . "', `title`='" . $db->escape($this->title) .
                "', `answer`='" . $db->escape($this->answer) . "', `submit`='" . $db->escape($this->submit) .
                "', `fields`='" . $db->escape($fields) . "', `hidden`='" . intval($this->status) . "' WHERE `id`='" .
                intval($this->id) . "'";
            $db->query($query);
            $this->n++;
        }
        ...
        $db->query("UPDATE `fp_forms` SET `description`='" . $db->escape($this->description) . "' WHERE `id`='" .
                   intval($this->id) . "'");
        $this->n++;
    }
