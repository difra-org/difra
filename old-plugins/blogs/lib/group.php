...
class Group {
	 ...
	 * @param array $data
	 */
	public function addUser( $id, $data = array() ) {

		$validKeys = array( 'role', 'confirmed', 'comment' );
		$db        = Difra\MySQL::getInstance();
		$keys      = array(
			"`group`='{$this->id}'",
			"`user`='" . intval( $id ) . "'"
	...
	public function delUser( $id ) {
		...
		$db = Difra\MySQL::getInstance();
		$db->query( "DELETE FROM `groups_users` WHERE `user`='" . $db->escape( $id ) . "' AND `group`='" . $db->escape( $this->id ) . "'" );
		$this->users = null;
	}
	...
	public function hasUser( $id ) {
		...
		return (bool) $db->fetchOne(
			"SELECT count(*) FROM `groups_users` WHERE `group`='{$this->id}' AND `user`='" . $db->escape( $id ) . "' AND `confirmed`=1" );
	}
	...
	public function setUserRole( $user, $role ) {
		...
		$db = Difra\MySQL::getInstance();
		$db->query( "UPDATE `groups_users` SET `role`='" . $db->escape( $role ) . "' WHERE `group`='{$this->id}' AND `user`='" . $db->escape( $user )
			    . "'" );
		$this->users = null;
	}
	...
	public function setUserConfirmed( $id, $confirmed = true ) {
		...
		$db        = Difra\MySQL::getInstance();
		$db->query( "UPDATE `groups_users` SET `confirmed`='{$confirmed}' WHERE `group`='{$this->id}' AND `user`='" . $db->escape( $id ) . "'" );
	}
	...
	public function delAllUsers( $confirmed = false ) {
		...
		$confirmed = $confirmed ? '1' : '0';
		$db        = \Difra\MySQL::getInstance();
		$db->query( "DELETE FROM `groups_users` WHERE `confirmed`='" . $db->escape( $confirmed ) . "' AND `group`='" . $db->escape( $this->id ) . "'" );
		$this->users = null;
	}
