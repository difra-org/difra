...
class Publication {
    ...
    private function save() {
        ...
        if( is_null( $this->id ) ) {
            ...
            $query = "INSERT INTO `news` (`link`, `title`, `pubDate`, `viewDate`, `stopDate`, `source`, `sourceURL`, `visible`, `important`)
                        VALUES ('" . $db->escape( $this->link ) . "', '" . $db->escape( $this->title ) . "', '" . $db->escape( $this->pubDate ) .
    ...
    private function saveHTMLTexts() {
        ...
        if( $this->announcement instanceof \Difra\Param\AjaxHTML || $this->announcement instanceof \Difra\Param\AjaxSafeHTML ) {
        ...
        $db = \Difra\MySQL::getInstance();
        $query = "UPDATE `news` SET `body`='" . $db->escape( $this->body ) . "', `announcement`='" . $db->escape( $this->announcement ) .
