...
class Users
{
    ...
    public function register($data)
    {
        ...
        $query = "INSERT INTO `users` SET `email`='" . $mysql->escape($data['email']) . "', `password`='" .
                 md5($data['password1']) . "'";
    ...
    public function login($email, $password, $remember, $withAdditionals = false)
    {
        ...
        if ($withAdditionals == true) {
            $additionalsData =
                $mysql->fetch("SELECT `name`, `value` FROM `users_fields` WHERE `id`='" . intval($data['id']) . "'");
    ...
    public function recoverSetPassword($key, $pw1, $pw2)
    {
        ...
        $db->query(
            'UPDATE `users_recovers` SET `used`=1,`date_used`=\'' . date('Y-m-d H:i:s') . "' WHERE `id`='" .
            $db->escape($key) . "'"
        );
        return true;
    }
    ...
    {
        return md5($password) == Auth::getInstance()->data['password'];
    }
    ...
    public function getUserXML(\DOMNode $node, $id)
    {
        ...
        $mysql->fetchXML(
            $node,
            'SELECT `id`,`email`,`active`,`banned`,`registered`,`logged`,`info`,`moderator` FROM `users` WHERE `id`=' .
            $mysql->escape($id)
        );
    ...
    public function setUserLogin($id, $data)
    {
        ...
        $mysql->query(
            "UPDATE `users` SET `email`='$email'" . ($passwd ? ",`password`='" . md5($passwd) . "'" : '') .
            ' WHERE `id`='
            . $mysql->escape($id)
        );
    ...
    public function checkUniqueFields($field, $data)
    {
        ...
        $mysql = MySQL::getInstance();
        $res = $mysql->fetch(
            'SELECT `id` FROM `users_fields` WHERE `name`=\'' . $mysql->escape($field) .
    ...
    private function _setLongSession($id)
    {
        ...
        $db->query(
            "REPLACE INTO `users_sessions` SET `id`='" . intval($id) .
            ...
            if (!empty($t)) {
                ...
                if (($data['ip'] & ip2long(self::IP_MASK)) == ip2long($currentNetwork)) {
                    ...
                    $additionalsData = $db->fetch(
                        "SELECT `name`, `value` FROM `users_fields` WHERE `id`='" .
                        intval($data['id']) . "'"
                    );
                    if (!empty($additionalsData)) {
                        foreach ($additionalsData as $k => $tempData) {
